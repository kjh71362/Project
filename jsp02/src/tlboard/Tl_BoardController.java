...
import tlboard.dao.Tl_BoardDAO;
import tlboard.dto.Tl_BoardDTO;


//@WebServlet("/board_servlet/*") 어노테이션은 꼭 지워야 함(web.xml에서 매핑 사용시)
public class Tl_BoardController extends HttpServlet {
	...
	IOException {
		...
		Tl_BoardDAO dao=new Tl_BoardDAO();
		
		if(url.indexOf("list.do") != -1) {
			List<Tl_BoardDTO> list=dao.list();
			request.setAttribute("list", list);
			String page="/hiphop/list.jsp";
			...
			dao.insert(dto);
			String page="/tlboard_servlet/list.do";
			...
			//번호에 해당하는 게시물 리턴
			Tl_BoardDTO dto=dao.viewReplace(num);
			//request 영역에 저장
			request.setAttribute("dto", dto);
			//화면 전환
			String page="/hiphop/view.jsp";
			...
			//댓글 목록 리턴
			List<Tl_BoardCommentDTO> list=dao.commentList(num);
			//request 영역에 저장
			request.setAttribute("list", list);
			//출력 페이지로 이동
			...
			Tl_BoardCommentDTO dto=new Tl_BoardCommentDTO();
			//게시물 번호
			...
			int num=Integer.parseInt(request.getParameter("num"));
			Tl_BoardDTO dto=dao.view(num);
			dto.setContent("===게시물의 내용===\n"+dto.getContent());
			request.setAttribute("dto", dto);
			String page="/hiphop/reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			if(result != null){ //비밀번호가 맞으면
				page="/hiphop/edit.jsp";
				request.setAttribute("dto",  dao.view(num));
				RequestDispatcher rd
					=request.getRequestDispatcher(page);
				rd.forward(request, response);
			}else{ //비밀번호가 틀리면
				page=contextPath+"/tlboard_servlet/view.do?num="
			+num+"&message=error";
